...
- **群组概览**：各群人数、规模占比、状态，可点开查看成员
- **成员明细**：QQ 头像、QQ 号 / 昵称、群名片 / 头衔、角色、加入时间、最后发言，支持**点击表头排序**、搜索、**导出 CSV**，也可直接**踢出成员**（二次确认，不可逆，需机器人具备相应群权限）
- **跨群重复分析**：规模占比环形图、在群数分布柱状图、群两两重叠 Top（可下钻看两群**共同成员明细**）、跨群最多成员 Top
- **群人数增长**：两条曲线叠加——「按加入时间累计」反映留存、「实际人数」从启用起按天落盘；可按年 / 月 / 日切换粒度，支持自定义起止日期
- **群成员活跃度**：按最后发言时间分成 活跃 / 较活跃 / 沉默 / 无记录（阈值可配置），给出活跃率与潜水成员 Top；选中单个群时叠加 QQ 原生活跃榜（当前龙王、群聊之火 等）
- **统计范围切换**：默认「仅统计群」；切到「全部群」会把机器人所在但未纳入的群也计入去重 / 重复率 / 重叠分析
- **按 QQ 检索**：输入任意 QQ 号，实时查出该号在机器人的哪些群、每个群的角色 / 名片 / 加入时间
- **发现 / 管理群**：列出机器人加入的所有群，网页里直接纳入 / 移出统计、或让机器人**退群**（二次确认，不可逆），并标出已退群的孤儿统计项
